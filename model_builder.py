...
class DualFocusVideoQA(nn.Module):
    ...
    def forward(self,
        ...
        question_token_ids = question_tokens['input_ids'].to(self.device) # Shape: [B, L_q_instr]
        instruct_masks_qvit = question_tokens['attention_mask'].to(self.device) # Shape: [B, L_q_instr]
        ...
        with torch.cuda.amp.autocast(enabled=(self.dtype == torch.float16 or self.dtype == torch.bfloat16)):
            ...
            for video_frames in m_video_frames:
                ...
                feat = self.vit_feature_extractor(video_frames,device=self.device).to(self.device)
        ...
        for i in range(batch_size):
            ...
            if is_training:
                ...
                labels_single[:, answer_start_index_in_final:answer_end_index_in_final] = answer_ids.to(self.device)
        ...
        # del all_r_frames, pixel_values_qvit_batch, instruct_states_qvit_expanded, instruct_masks_qvit_expanded
        # del qvit_output_frames, s_feat, s_feat_list
        # del t_feat_list  # Potentially t_feat if it was concatenated
        # del p_feat_list, all_inputs_embeds, all_labels, final_inputs_embeds, final_attention_mask, final_labels
        # Add any other large intermediate tensors if necessary
        # torch.cuda.empty_cache()  # Force release memory (use cautiously, might slow down)
...
if __name__=='__main__':
    ...
    for i, batch in enumerate(dataloader):
        ...
        try:
            ...
            outputs_train = model(r_video_frames=batch['video_r'], m_video_frames=batch['video_m'], questions=batch['question'], answers=batch['answer'])
        ...
        try:
            model.eval() # 设置为评估模式
            outputs_infer = model(r_video_frames=batch['video_r'], m_video_frames=batch['video_m'], questions=batch['question'], answers=None) # 无答案
